...
*   **Local Setup & DX (Docker):** I assumed other developers would appreciate a frictionless setup, so I included a `docker-compose.yml` that automatically initializes the database and mounts the schema on startup.
...
However, I explicitly directed the AI to write the authorization middleware rather than just returning a simple controller. Furthermore, the AI originally generated a standard `SELECT * FROM orders` query. I overrode this by explicitly ensuring we created a **composite database index on `(user_id, created_at DESC)`** and wrote parameterized SQL to leverage that index for efficient sorting. I also directed the inclusion of an `express-rate-limit` middleware.

### 3. What breaks first at 100x this data?
At 100x the data (5,000,000 orders), two primary systems will break:
1.  **Database Pagination (Offset):** Using `LIMIT X OFFSET Y` becomes progressively slower as the dataset grows because PostgreSQL must scan and discard `Y` rows before returning `X`. 
    ...
    *   **Fix:** Migrate from Offset pagination to **Cursor-based pagination** (e.g., `WHERE created_at < last_seen_created_at LIMIT 10`).
...
*   **An ORM (Prisma/TypeORM):** Deliberately used raw `pg` to demonstrate an understanding of the underlying SQL queries and indexes, rather than hiding them behind an abstraction.
